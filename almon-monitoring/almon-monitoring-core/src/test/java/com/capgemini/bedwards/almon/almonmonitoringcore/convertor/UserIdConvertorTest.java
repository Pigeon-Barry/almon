...
@ContextConfiguration(
        classes = {
                UserIdConvertor.class
        }
)
public class UserIdConvertorTest {
    @Autowired
    private UserIdConvertor convertor;
    ...
    public void positive_convert(UUID source) {
        ...
        doReturn(expected).when(service).getUserById(eq(source));
        assertEquals(expected, convertor.convert(source));

        verify(service, times(1)).getUserById(eq(source));
    }
}
